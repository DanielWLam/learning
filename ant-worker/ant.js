 ...
 * @Last Modified time: 2015-12-18 16:28:38
 */
...
function onTaskDone(err, task) {
    ...
    if (!task.harvest.honey.hasOwnProperty()) {
        harvest.create(task, function(err, res) {
            if (err) {
                console.log(err);
                return;
            }
            console.log(res + '已保存到flower表');
        });
        task.harvest.flower.forEach(function(item) {
            run({
                url: item.url,
                done: flowerDone
            })
...
function flowerDone(err, task) {
    if (err) {
        logger.info(err);
        return;
    }
    harvest.saveHoney(task, function(err, res) {
        if (err) {
            console.log(err);
            return;
        }
        console.log(res + '已保存到honey表')
    })
}
