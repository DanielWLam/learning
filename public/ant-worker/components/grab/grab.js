    ...
    request(options, function (err, res, body) {
        if (err || !res) {
            if (err.code === 'ETIMEDOUT') {
                reject(new NetworkError('CONNECT_TIMEOUT'));
            } else if (err.code === 'ECONNREFUSED') {
                reject(new NetworkError('CONNECT_REFUSED'));
            } else if (err.message.indexOf('Invalid URI') > -1) {
                reject(new NetworkError('INVALID_URI'));
            } else {
                reject(new NetworkError(err.code));
            }
            return;
        }
        ...
        try {
            ...
            reject(new HtmlError('DECODE_ERROR'));
            return;
        }
...
function parseHtml(context, options) {
    return new Promise(function (resolve, reject) {
        ...
        try {
            var head = content.substring(0, 200).trim().toLowerCase(),
                rootTag = 'html';
...
function extractValue($, $dom, selector, options) {
    try {
        ...
        console.log('EXTRACT_VALUE_ERROR', err.message, err.stack);
        throw err;
    }
}
    ...
    return new Promise(function (resolve, reject) {
        if (selector) {
            if (context.contentType === 'html') {
                ...
                console.log(sign, context.data[selector], sign);
            }
        } else {
            console.log(sign, context.content, sign);
        }
    });
};
...
Grab.prototype.next = function () {
    ...
    if (fn) {
        ...
        .catch(function (err) {
            // 打印出错误信息
            that._log('CATCH_PROMISE_EXCEPTION', err.message, err.stack);
...
Grab.prototype.log = function (fn) {
    ...
    return new Promise(function (resolve, reject) {
        if (fn) {
            ...
            reject(new RuntimeError('LOG_FN_UNDEFINED'));
        }
    });
};
